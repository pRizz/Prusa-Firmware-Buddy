...
EXPECTED_ARCHIVES = {
    ...
    "mini404_linux_x86_64": {
        "sha256":
        "2709a43dbb6e64ea4597399d2f0e05be13e70a7b659a18ac61512498d320a5ba",
...
FORBIDDEN_DECLARATION_PATTERNS = {
    ...
    "PATH lookup":
    r"(?i)(?:repository_ctx\.which|ctx\.which|\bwhich\s+|\bPATH\b)",
...
def _workspace_file(relative_path: str) -> Path:
    source_workspace_root = Path(__file__).resolve().parents[3]
    return source_workspace_root / relative_path


def _load_inputs() -> ProvenanceInputs:
    return ProvenanceInputs(
        bazel_version=_workspace_file(".bazelversion").read_text(
            encoding="utf-8"),
        lock=json.loads(
        ...
        repositories=_workspace_file(
            "tools/bazel/toolchains/embedded_repositories.bzl").read_text(
                encoding="utf-8"),
    )
...
def validate_provenance(inputs: ProvenanceInputs) -> list[str]:
    ...
    for module_name, expected_version in EXPECTED_MODULE_VERSIONS.items():
        ...
        if declaration.search(inputs.module) is None:
            errors.append(
                f"{module_name} must be pinned to {expected_version}")
    ...
    for fragment in required_module_fragments:
        if fragment not in inputs.module:
            errors.append(
                f"MODULE.bazel is missing exact declaration: {fragment}")
    ...
    for hash_value in hash_values:
        if re.fullmatch(r"[0-9a-f]{64}", hash_value) is None:
            errors.append(
                "archive SHA-256 must be a non-placeholder lowercase digest")
        elif len(set(hash_value)) == 1:
            errors.append(
                "archive SHA-256 must not be a repeated placeholder digit")
    ...
    if re.search(r"(?i)(?:darwin|macos|osx)", inputs.repositories) is not None:
        errors.append("forbidden Darwin embedded archive fallback")
    ...
    for required_lock_value in (
        ...
        if required_lock_value not in lock_text:
            errors.append(
                f"MODULE.bazel.lock is missing {required_lock_value}")
...
class ToolchainProvenanceTest(unittest.TestCase):
        ...
        for exact_version in (
            ...
            with self.subTest(exact_version=exact_version):
                ...
                mutated = ProvenanceInputs(
                    ...
                    module=self.inputs.module.replace(exact_version,
                                                      "99.99.99"),
    ...
    def test_each_forbidden_fallback_class_is_rejected(self) -> None:
        forbidden_mutations = (
            ("module", 'new_local_repository(name = "ambient")'),
            ("module", 'repository_ctx.which("arm-none-eabi-gcc")'),
            ("module", 'tool_path = "/tmp/.dependencies/arm-none-eabi-gcc"'),
            ("repositories",
             'url = "https://example.invalid/tool-darwin.tar.xz"'),
            ("module", 'command = "cargo build"'),
            ("module", 'command = "cmake --build output"'),
            ("module", 'source = "tools/bazel/fixtures/firmware.bin"'),
            ("module", 'source = ".planning/archive/firmware.elf"'),
            ("module", 'source = "release/archives/firmware.elf"'),
        )
        for mutation_target, mutation in forbidden_mutations:
            with self.subTest(mutation_target=mutation_target,
                ...
                mutated = ProvenanceInputs(
                    ...
                    module=(f"{self.inputs.module}\n{mutation}\n"
                            if mutation_target == "module" else
                            self.inputs.module),
                    repositories=(f"{self.inputs.repositories}\n{mutation}\n"
                                  if mutation_target == "repositories" else
                                  self.inputs.repositories),
                )
    ...
    def test_placeholder_hashes_are_rejected(self) -> None:
        for placeholder_hash in ("", "0" * 64, "sha256-placeholder"):
            with self.subTest(placeholder_hash=placeholder_hash):
                # Arrange
                original_hash = EXPECTED_ARCHIVES["arm_gnu_linux_x86_64"][
                    "sha256"]
                mutated = ProvenanceInputs(
                    bazel_version=self.inputs.bazel_version,
                    lock=self.inputs.lock,
    ...
    def test_declared_python_interpreter_is_exact_and_not_ambient(
            self) -> None:
        # Arrange
        interpreter_path = Path(sys.executable).as_posix()
        toolchain_prefix = Path(sys.base_prefix).as_posix()
        ...
        self.assertIn("python_3_12_10", toolchain_prefix)
