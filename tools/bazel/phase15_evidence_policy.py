...
REQUIRED_SUPPORTED_PRINTER_FAMILIES = {
...
SOURCE_CONTRACT_ALLOWED_STATUSES = {
    "source-contract-passed", "failed", "rejected-redaction",
    "rejected-overclaim"
}
...
def value_covers(required_value: str, actual_value: str, field: str) -> bool:
    ...
    if field == "board" and actual_value == "all-master-boards":
        return required_value in {
            "BUDDY", "XBUDDY", "XLBUDDY", "XL_DEV_KIT_XLB"
        }
    return False


def covered_values(scenarios: list[dict[str, Any]], field: str,
                   required_values: set[str]) -> set[str]:
    covered: set[str] = set()
...
def check_operator_schema(contract: dict[str, Any], errors: list[str]) -> None:
    ...
    if required_fields != REQUIRED_OPERATOR_FIELDS:
        errors.append(
            "operator_input_schema required_fields must match required operator metadata fields"
        )
    if set(allowed_results) != {
            "passed", "failed", "blocked-hardware-unavailable"
    }:
        errors.append(
            "operator_input_schema allowed_results must be passed, failed, blocked-hardware-unavailable"
        )
...
def check_contract(root: Path) -> dict[str, Any]:
    ...
    if status_vocabulary != STATUS_VOCABULARY:
        errors.append(
            "status_vocabulary does not match the Phase 15 vocabulary")
    ...
    for scenario in scenarios:
        ...
        if proof_scope == "hardware-observation":
            missing_statuses = sorted(HARDWARE_ALLOWED_STATUSES -
                                      allowed_statuses)
            if missing_statuses:
                errors.append(
                    f"{row_name} hardware allowed_statuses missing: {', '.join(missing_statuses)}"
                )
            ...
            errors.append(
                f"{row_name} proof_scope must be hardware-observation or source-contract"
            )
    ...
    if missing_requirements:
        errors.append("missing HARD requirement coverage: " +
    ...
    for field, required_values in coverage_checks:
        missing_values = sorted(
            required_values -
            covered_values(scenarios, field, required_values))
        if missing_values:
            errors.append(
                f"missing required {field} coverage: {', '.join(missing_values)}"
            )
    if errors:
        raise VerificationError("\n".join(errors))
    return contract
