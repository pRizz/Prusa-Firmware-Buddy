...
FILE_SECRET_PATTERNS = [
    ...
    (re.compile(r"\b(?:api[_-]?key|token|credential|password|secret)\s*[:=]",
...
def is_safe_relative_path(value: str) -> bool:
    path = Path(value)
    return not path.is_absolute() and ".." not in path.parts and value.strip(
    ) == value and value != ""


def ensure_safe_repo_relative(value: str, context: str,
...
def validate_output_dir(root: Path, output_dir: Path) -> Path:
    if output_dir.is_absolute() or ".." in output_dir.parts:
        raise VerificationError(
            f"output directory must stay under {DEFAULT_OUTPUT_DIR}: {output_dir}"
        )
    ...
    for part in output_dir.parts:
        ...
        if current.exists() and current.is_symlink():
            raise VerificationError(
                f"output directory contains symlink component: {current.relative_to(root)}"
            )
...
def validate_generated_artifacts(contract: dict[str, Any],
                                 errors: list[str]) -> None:
    artifacts = require_list(contract.get("generated_artifacts"),
                             "generated_artifacts")
    ...
    for artifact in artifacts:
        if not isinstance(artifact, str):
            errors.append(
                f"generated_artifacts contains non-string entry: {artifact!r}")
            continue
        if not is_safe_relative_path(artifact):
            errors.append(f"generated_artifacts unsafe path: {artifact}")


def validate_source_refs(row_id: str, source_refs: Any,
                         errors: list[str]) -> None:
...
def validate_correction_rows(contract: dict[str, Any],
                             errors: list[str]) -> None:
    rows = require_list(contract.get("metadata_corrections"),
    ...
    for index, raw_row in enumerate(rows):
        ...
        missing = [
            field for field in CORRECTION_REQUIRED_FIELDS
        ...
        if isinstance(target_file, str):
            ensure_safe_repo_relative(target_file, f"{row_id} target_file",
                                      errors)
    ...
    missing_ids = [
        row_id for row_id in REQUIRED_CORRECTION_IDS
        if row_id not in rows_by_id
    ]
    if missing_ids:
        errors.append(
            f"metadata_corrections missing required rows: {', '.join(missing_ids)}"
        )
...
def validate_debt_rows(contract: dict[str, Any], errors: list[str]) -> None:
    ...
    for index, raw_row in enumerate(rows):
        ...
        missing = [
            field for field in DEBT_REQUIRED_FIELDS
...
def validate_audit_gap_rows(contract: dict[str, Any],
                            errors: list[str]) -> None:
    rows = require_list(contract.get("audit_gap_mappings"),
                        "audit_gap_mappings")
    rows_by_id: dict[str, dict[str, Any]] = {}
    for index, raw_row in enumerate(rows):
        if not isinstance(raw_row, dict):
        ...
        rows_by_id[row_id] = row
        for field in [
                "id", "original_audit_gap", "mapped_status", "source_refs",
                "correction_ids", "no_overclaim_rationale"
        ]:
            if row.get(field) in (None, "", []):
    ...
    missing_ids = [
        row_id for row_id in REQUIRED_AUDIT_GAP_IDS if row_id not in rows_by_id
    ]
    if missing_ids:
        errors.append(
            f"audit_gap_mappings missing required rows: {', '.join(missing_ids)}"
        )


def check_contract(root: Path) -> dict[str, Any]:
    contract = require_dict(load_json(root / CONTRACT_MANIFEST),
    ...
    if not isinstance(snapshots, list) or not snapshots:
        errors.append("sanitized_source_snapshots must be a non-empty list")
            ...
            isinstance(snapshot, str) and is_safe_relative_path(snapshot)
    ...
    for text in iter_strings(contract):
        ...
        for marker in markers:
            if marker in lowered:
                errors.append(
                    f"forbidden marker {marker} in contract text: {text}")
    if errors:
        raise VerificationError("\n".join(errors))


def check_file_security_text(path: Path, text: str,
                             broad_markers: bool) -> list[str]:
    errors: list[str] = []
...
def generated_artifact_paths(root: Path, output_dir: Path,
                             contract: dict[str, Any]) -> list[Path]:
    paths: list[Path] = []
    for artifact in require_list(contract.get("generated_artifacts"),
                                 "generated_artifacts"):
...
def check_security(root: Path, output_dir: Path) -> None:
    ...
    for path in target_metadata_paths(root, contract):
        text = read_text(path)
        errors.extend(
            check_file_security_text(path.relative_to(root),
                                     text,
                                     broad_markers=False))
...
def requirement_rows(text: str, requirement_id: str) -> list[str]:
    return [
        ...
        if re.search(rf"\|\s*{re.escape(requirement_id)}\s*\|", line)
    ]
...
def check_requirements(root: Path) -> None:
    ...
    for requirement_id, caveat in caveats.items():
        if re.search(rf"-\s+\[ \]\s+\*\*{re.escape(requirement_id)}\*\*",
                     text):
        ...
        for row in requirement_rows(text, requirement_id):
            ...
            if re.search(r"\bComplete\b", row,
                         re.IGNORECASE) and caveat not in row:
                errors.append(
                    f"{requirement_id}: Complete row must include caveat: {caveat}"
                )
...
def validation_path_for_phase(root: Path, phase: int) -> Path:
    matches = sorted(
        (root /
         ".planning/phases").glob(f"{phase:02d}-*/{phase:02d}-VALIDATION.md"))
    if not matches:
...
def has_unchecked_wave_zero_bullet(text: str) -> bool:
    ...
    for line in text.splitlines():
        if line.startswith("#"):
            in_wave_zero = bool(
                re.search(r"\bwave\s*0\b|\bw0\b", line, re.IGNORECASE))
            continue
    ...
    for phase in PHASES_WITH_VALIDATION_METADATA:
        ...
        for line in text.splitlines():
            if line.strip().startswith("|") and re.search(
                    r"\|\s*pending\s*\|", line, re.IGNORECASE):
...
def check_roadmap_state(root: Path) -> None:
    ...
    if re.search(r"\|\s*21\.[^|\n]*\|\s*v?1\.1\s*\|\s*0/0\s*\|\s*Planned\s*\|",
                 roadmap):
        errors.append("Phase 21 roadmap row is stale: 0/0 | Planned")
    if re.search(r"\|\s*22\.[^|\n]*\|\s*v?1\.1\s*\|\s*0/3\s*\|\s*Planned\s*\|",
                 roadmap):
    ...
    for line in state.splitlines():
        ...
        if "current focus" in lowered or "current position" in lowered:
            if "phase 20" in lowered or "phase 21" in lowered:
                errors.append(
                    f"STATE current-position text is stale: {line.strip()}")
...
def check_audit_readiness(root: Path,
                          metadata_corrected: bool = False) -> dict[str, Any]:
    contract = check_contract(root)
    gap_rows = require_list(contract.get("audit_gap_mappings"),
                            "audit_gap_mappings")
    debt_rows = require_list(contract.get("non_blocking_debt"),
                             "non_blocking_debt")
    allowed_statuses = {
        "closed", "still_blocking", "non_blocking_debt",
    ...
    for raw_row in gap_rows:
        ...
        normalized_rows.append({
            ...
            "status":
            normalized_status,
            "original_mapped_status":
            status,
            "source_refs":
            raw_row.get("source_refs", []),
            "correction_ids":
            raw_row.get("correction_ids", []),
        })
