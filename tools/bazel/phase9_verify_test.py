...
from phase9_contract_policy import (
    FORBIDDEN_MARKERS,
    OVERCLAIM_STRINGS,
    PHASE,
    PHASE_LIFECYCLE_ID,
...
class Phase9VerifierFixture:
    ...
    def make_temp_root(self) -> tuple[tempfile.TemporaryDirectory[str], Path]:
        ...
        shutil.copy2(ROOT / "tools/bazel/phase9_contract_policy.py",
                     root / "tools/bazel/phase9_contract_policy.py")
        if NEGATIVE_RUNNER.exists():
            shutil.copy2(NEGATIVE_RUNNER,
                         root / "tools/bazel/phase9_negative_fixtures.py")
    ...
    def wui_row(self, root: Path, row_id: str) -> dict[str, object]:
        sources_by_id = {
            ...
            ["lib/WUI/nhttp/server.cpp", "lib/WUI/nhttp/README.md"],
    ...
    def network_service_row(self, root: Path,
                            row_id: str) -> dict[str, object]:
        row = self.base_contract_row(
            root,
            row_id,
            "IFCE-03",
    ...
    def concern_row(self, root: Path, row_id: str) -> dict[str, object]:
        ...
        source_paths = sources_by_id.get(row_id,
                                         [".planning/codebase/CONCERNS.md"])
        self.write_source_paths(root, source_paths)
        concern_prefix = "concern-phase9-"
        concern_id = row_id[len(concern_prefix):] if row_id.startswith(
            concern_prefix) else row_id
        row: dict[str, object] = {
            "id": row_id,
            ...
            "intentional_delta": "none",
            "regression_guard":
            ["future work must keep the disposition explicit"],
            "secret_handling": "none",
            "phase_lifecycle_id": PHASE_LIFECYCLE_ID,
        }
        ...
        if row_id == "concern-phase9-crash-dump-upload-boundary":
            row["regression_guard"] = [
                "consent", "redaction", "no raw dump byte payloads"
            ]
            row["secret_handling"] = "named-only-redacted"
    ...
    def write_rust_api_surface(
        ...
        self.write_file(
            ...
            or ("#![forbid(unsafe_code)]\n"
                "pub mod network;\n"
                "pub use network::{" + ", ".join(RUST_API_STRINGS) + "};\n"),
        )
...
class Phase9VerifierTest(Phase9VerifierFixture, unittest.TestCase):

    def test_help_lists_phase9_modes(self) -> None:
        # Act
...
if __name__ == "__main__":
    import phase9_verify_failure_test

    loader = unittest.TestLoader()
    suite = loader.loadTestsFromTestCase(Phase9VerifierTest)
    suite.addTests(
        loader.loadTestsFromTestCase(
            phase9_verify_failure_test.Phase9VerifierFailureTest))
