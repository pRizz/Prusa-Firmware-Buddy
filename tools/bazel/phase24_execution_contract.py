...
def phase15_scenarios(contract: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(scenarios, list):
        raise VerificationError(
            f"{PHASE15_CONTRACT.as_posix()} must contain a scenarios list")
    parsed: list[dict[str, Any]] = []
    for index, scenario in enumerate(scenarios):
        if not isinstance(scenario, dict):
            raise VerificationError(
                f"{PHASE15_CONTRACT.as_posix()} scenarios[{index}] must be an object"
            )
...
def source_status_vocabulary(phase15: dict[str, Any]) -> set[str]:
    return set(
        require_list_of_strings(phase15, "status_vocabulary",
                                "phase15 contract"))
...
def check_contract(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if status_vocabulary != V1_2_STATUSES:
        errors.append(
            "status_vocabulary must match v1.2 hardware evidence statuses")
    if blocking_source_statuses != BLOCKING_SOURCE_STATUSES:
        errors.append(
            "blocking_source_statuses must match Phase 15 non-passing hardware statuses"
        )
    if requirement_ids != REQUIRED_REQUIREMENT_IDS or upstream_requirement_ids != REQUIRED_REQUIREMENT_IDS:
        errors.append(
            "requirement_ids and upstream_result_row requirement_ids must be EVID-02"
        )
    ...
    if source_contracts != expected_source_contracts:
        errors.append(
            "source_contracts must name Phase 15, Phase 18, Phase 19, and Phase 23 contracts"
        )
    if allowed_artifact_roots != {
            DEFAULT_OUTPUT_DIR.as_posix() + "/", "external://phase24/"
    }:
        errors.append(
            "allowed_artifact_roots must be build/ci-evidence/phase24/ and external://phase24/"
        )
    ...
    if upstream.get("criterion_id") != "final-hardware-safety-media-evidence":
        errors.append(
            "upstream_result_row criterion_id must be final-hardware-safety-media-evidence"
        )
    ...
    if sorted(required_scenario_ids) != sorted(scenario_ids):
        errors.append(
            "required_phase15_scenario_ids must exactly match Phase 15 scenarios"
        )
    ...
    for scenario in scenarios:
        ...
        try:
            ...
            if not allowed_statuses <= phase15_statuses:
                errors.append(
                    f"{row_name} contains unknown Phase 15 allowed statuses")
            for source_ref in require_list_of_strings(scenario,
                                                      "source_contract_refs",
                                                      row_name):
                if "#" not in source_ref or Path(source_ref.split(
                        "#", 1)[0]).is_absolute() or ".." in Path(
                            source_ref.split("#", 1)[0]).parts:
                    errors.append(
