...
class Phase14SimulatorEvidenceTest(unittest.TestCase):
    ...
    def make_temp_root(self) -> tuple[tempfile.TemporaryDirectory[str], Path]:
        ...
        shutil.copy2(VERIFIER,
                     root / "tools/bazel/phase14_simulator_evidence.py")
        shutil.copy2(
            ROOT / "tools/bazel/phase14_evidence_policy.py",
            root / "tools/bazel/phase14_evidence_policy.py",
        )
    ...
    def write_wiring(
        ...
        self.write_file(
            ...
            maybe_build or """load(":shell_rules.bzl", "shell_binary")
...
""",
        ...
        self.write_file(
            root,
            "justfile",
            maybe_justfile or """phase14-verify:
...
""",
    ...
    def test_contract_only_rejects_path_traversal_artifact_path(self) -> None:
        ...
        with temp_dir:
            ...
            contract["scenarios"][0][
                "expected_artifact_path"] = "../phase14.log"
            self.write_contract(root, contract)
    ...
    def test_security_only_rejects_secret_marker_in_generated_artifact(
            self) -> None:
        # Arrange
        temp_dir, root = self.make_temp_root()
        with temp_dir:
            ...
            self.write_file(root,
                            "build/ci-evidence/phase14/redacted-summary.json",
                            "token_value\n")
    ...
    def test_quick_writes_artifacts_and_preserves_pending_inputs(self) -> None:
        ...
        with temp_dir:
            ...
            # Act
            result = self.run_verifier(["--quick", "--output-dir", output_dir],
                                       maybe_root=root)
            manifest = json.loads(
                (root / output_dir /
            ...
            active_statuses = {
                ...
                for row in manifest["scenarios"] if row["scenario_id"] !=
            ...
            self.assertEqual(result.returncode, 0, result.stdout)
            self.assertTrue(
                (root / output_dir / "normalized-scenarios.json").exists())
            self.assertTrue(
                (root / output_dir /
            ...
            self.assertTrue(
                (root / output_dir /
                 "logs/sim-gcode-file-print-telemetry.log").exists())
            self.assertEqual(set(active_statuses.values()),
    ...
    def test_real_command_is_argument_list(self) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            "phase14_simulator_evidence", VERIFIER)
    ...
    def test_redacted_command_for_log_uses_basenames_for_input_paths(
            self) -> None:
        # Arrange
        spec = importlib.util.spec_from_file_location(
            "phase14_simulator_evidence", VERIFIER)
    ...
    def test_real_mode_log_redacts_command_path_markers(self) -> None:
        ...
        with temp_dir:
            ...
            result = self.run_verifier(
                [
                    "--run-simulator", "--firmware",
                    firmware.as_posix(), "--output-dir", output_dir
                ],
                maybe_root=root,
            )
            retained_text = "\n".join(
...
""",
    ...
    def test_wiring_only_rejects_missing_bazel_labels(self) -> None:
        ...
        with temp_dir:
            self.copy_complete_surface(root)
            build_text = (root / "tools/bazel/BUILD.bazel").read_text(
