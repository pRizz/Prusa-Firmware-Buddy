...
from phase5_runtime_policy import *  # noqa: F403
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise VerificationError(
            f"{path} is not valid JSON: {error}") from error
...
def require_top_level(data: dict[str, Any], path: Path,
    ...
    if not isinstance(rows, list):
        raise VerificationError(
            f"{path} must contain a {collection_name} list")
...
def require_unique(rows: list[dict[str, Any]], field: str,
    ...
    for row in rows:
        ...
        if not isinstance(value, str):
            raise VerificationError(
                f"{path} row has non-string {field}: {value!r}")
        if value in values:
    ...
    if duplicates:
        raise VerificationError(
            f"{path} has duplicate {field} values: {', '.join(sorted(duplicates))}"
        )
    return values
...
def require_requirements(row: dict[str, Any], row_name: str,
                         allowed: set[str]) -> None:
    requirements = row.get("requirements")
    if not isinstance(requirements, list) or not all(
            isinstance(item, str) for item in requirements):
        raise VerificationError(
            f"{row_name} requirements must be a list of strings")

    if not set(requirements).intersection(allowed):
        raise VerificationError(
            f"{row_name} requirements must include at least one of {', '.join(sorted(allowed))}"
        )


def require_text(row: dict[str, Any], row_name: str,
                 needles: list[str]) -> None:
    haystack = json.dumps(row, sort_keys=True)
    missing = [needle for needle in needles if needle not in haystack]
    if missing:
        raise VerificationError(
            f"{row_name} missing required evidence text: {', '.join(missing)}")
...
def audit_source_paths(rows: list[dict[str, Any]]) -> set[str]:
    return {
        row["source_path"]
        for row in rows if isinstance(row.get("source_path"), str)
    }


def require_exact_audit_source_row(rows: list[dict[str, Any]],
                                   source_path: str, surface_id: str) -> None:
    for row in rows:
        if row.get("source_path") == source_path and row.get(
...
def check_unsafe_audit_manifest() -> None:
    ...
    require_ids(source_paths, REQUIRED_AUDIT_SOURCE_PATHS,
                "adapter source paths")
    require_text(
...
def check_adapter_surface() -> None:
    ...
    for path in PURE_UNSAFE_FREE_FILES:
        ...
        if "#![forbid(unsafe_code)]" not in text:
            raise VerificationError(
                f"{path} must contain #![forbid(unsafe_code)]")
    ...
    for surface_id in REQUIRED_SYNC_SURFACE_IDS:
        if surface_id not in paths_by_surface_id:
            raise VerificationError(
                f"missing required unsafe-audit surface: {surface_id}")
    ...
    if findings:
        raise VerificationError("unaudited Rust unsafe surface found:\n" +
                                "\n".join(findings))
...
def check_bazel_surface() -> None:
    ...
    for needle in [
            "phase5_verify", "retained_foreign_code", "unsafe_boundary_audit"
    ]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify Phase 5 runtime boundary manifests")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--quick",
                      action="store_true",
                      help="Run inventory and unsafe-audit checks")
    mode.add_argument(
        "--all",
        action="store_true",
        help="Run quick checks plus Cargo format, lint, build, and tests")
    mode.add_argument("--inventory-only",
                      action="store_true",
                      help="Run only retained foreign-code inventory checks")
    mode.add_argument("--audit-only",
                      action="store_true",
                      help="Run only unsafe/runtime-boundary audit checks")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Phase 5 runtime boundary verification failed: {error}",
              file=sys.stderr)
        return 1
