...
extern "C" void HAL_UART_MspInit(UART_HandleTypeDef *huart) {
    ...
    if (huart->Instance == USART3) {
        ...
        auto GPIO_InitStruct = hal_config::uart_rs485_rx_tx_gpio();
        HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
        GPIO_InitStruct = hal_config::uart_rs485_de_gpio();
        HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
    ...
    if (huart->Instance == USART2) {
        ...
        auto GPIO_InitStruct = hal_config::uart_mmu_tx_gpio();
        HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
        __HAL_RCC_GPIOB_CLK_ENABLE();
        GPIO_InitStruct = hal_config::uart_mmu_rx_gpio();
        HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
...
void rs485_init() {
    hal_config::configure_uart(huart_rs485, USART3, 230'400);
...
void mmu_init() {
    hal_config::configure_uart(huart_mmu, USART2, 115'200);
...
extern "C" void HAL_ADC_MspInit(ADC_HandleTypeDef *hadc) {
    ...
    if (hadc->Instance == ADC1) {
        ...
        PB1     ------> ADC1_INP5
        */
        auto GPIO_InitStruct = hal_config::adc_gpio();
        HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
    }
}
static void tim1_postinit() {
    ...
    PA10    ------> TIM1_CH3
    */
    auto GPIO_InitStruct = hal_config::tim1_gpio();
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
...
static void tim2_postinit() {
    ...
    PA3     ------> TIM2_CH4
    */
    auto GPIO_InitStruct = hal_config::tim2_gpio();
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
}
static void tim3_postinit() {
    ...
    PB0     ------> TIM3_CH3
    */
    auto GPIO_InitStruct = hal_config::tim3_gpio_a();
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
    GPIO_InitStruct = hal_config::tim3_gpio_b();
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
}
static void tim1_init() {
    ...
    TIM1->CCMR1 = hal_config::tim1_ccmr1();
    // configure channel 3 & 4
    TIM1->CCMR2 = hal_config::tim1_ccmr2();
...
static void MX_ADC1_Init(void) {
    hal_config::configure_adc(hadc1);
    if (HAL_ADC_Init(&hadc1) != HAL_OK) {
        abort();
    }
    static constexpr auto single_diff = ADC_SINGLE_ENDED;
    auto sConfig = hal_config::adc_channel();
...
extern "C" void HAL_I2C_MspInit(I2C_HandleTypeDef *hi2c) {
    if (hi2c->Instance == I2C2) {
        ...
        PB13     ------> I2C2_SDA
        */
        auto GPIO_InitStruct = hal_config::i2c_gpio();
        HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
        /* Peripheral clock enable */
...
static void mmu_pins_init() {
    ...
    auto GPIO_InitStruct = hal_config::output_gpio(GPIO_PIN_13);
    HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
}
void usb_pins_init() {
    auto GPIO_InitStruct = hal_config::output_gpio(GPIO_PIN_2);
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
}
static void pub_init() {
    __HAL_RCC_GPIOC_CLK_ENABLE();
    auto GPIO_InitStruct = hal_config::output_gpio(GPIO_PIN_15);
    HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
...
static void filament_sensor_pins_init() {
    auto GPIO_InitStruct = hal_config::filament_sensor_gpio();
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
}
void hal::init() {
    HAL_Init();
...
#if EXTENSION_IS_IX()
    filament_sensor_state = (HAL_GPIO_ReadPin(GPIOA, hal_config::filament_sensor_pin()) == GPIO_PIN_SET) ? hal::filament_sensor::State::no_filament : hal::filament_sensor::State::has_filament;
#else
    filament_sensor_raw[filament_sensor_measuring_phase] = (HAL_GPIO_ReadPin(GPIOA, hal_config::filament_sensor_pin()) == GPIO_PIN_SET);
