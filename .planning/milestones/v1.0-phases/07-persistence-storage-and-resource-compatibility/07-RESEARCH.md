...
1. **Where should durable Phase 7 storage fixtures live?**
   ...
   - Previously unclear: No committed redacted EEPROM/config migration fixture corpus for Phase 7 was found in this research scan. [VERIFIED: local command `rg -n "EEPROM.*fixture|fixture.*EEPROM|storage fixture|storage.*fixture|migration fixture"`]
   ...
   - **RESOLVED:** Durable Phase 7 storage fixtures/catalog live under `tools/bazel/fixtures/phase7_storage/` as synthetic/redacted JSON fixture identity/catalog files. The planned catalog is `tools/bazel/fixtures/phase7_storage/redacted_migration_catalog.json`; it records fixture identity, source paths, row IDs, redaction policy, and compatibility facts for old EEPROM versions, current schema, settings import/export, credential redaction, selftest/calibration state, and journal hash facts, but not raw EEPROM bytes, byte arrays, passwords, tokens, certificates, private keys, or signing material.
...
2. **Should Phase 7 fix shell-generator safety or only disposition it?**
   ...
   - Previously unclear: The context does not mandate fixing these scripts during Phase 7. [VERIFIED: .planning/phases/07-persistence-storage-and-resource-compatibility/07-CONTEXT.md]
   ...
   - **RESOLVED:** Phase 7 shell-generator safety remains concern disposition plus label/static verification only. Current plans must not add a shell-script hardening implementation task unless execution intentionally creates a documented intentional delta with requirement mapping and regression evidence; the current Phase 7 plans preserve the concern as `preserve-with-explicit-risk`.
...
3. **How much generator execution should `just phase7-verify` run locally?**
   ...
   - Previously unclear: Whether the execution phase will run `utils/bootstrap.py` before Phase 7 generator checks. [VERIFIED: .planning/codebase/STACK.md]
   ...
   - **RESOLVED:** `just phase7-verify` runs deterministic static verifier, verifier regression tests, and Rust checks through the Phase 7 Bazel/just facade. Full generator execution, hardware/media execution, simulator flows, and physical storage proof remain bootstrap/CI/manual evidence outside the local quick pass unless later phases add normalized fixtures or an explicit bootstrap-backed generator verification task.
...
| IFCE-04 | Redacted storage migration catalog identities cover old EEPROM versions, current schema, settings import/export, credential redaction, selftest/calibration state, and journal hash facts without byte material or secrets. [VERIFIED: .planning/REQUIREMENTS.md; VERIFIED: .planning/phases/07-persistence-storage-and-resource-compatibility/07-CONTEXT.md] | static catalog/verifier | `python3 tools/bazel/phase7_verify.py --quick` | no, Wave 0. [VERIFIED: resolved Open Questions] |
...
- [ ] `tools/bazel/fixtures/phase7_storage/redacted_migration_catalog.json` - synthetic/redacted old EEPROM migration, current schema, settings import/export, credential redaction, selftest/calibration state, and journal hash fixture identities. [VERIFIED: resolved Open Questions; VERIFIED: .planning/phases/07-persistence-storage-and-resource-compatibility/07-CONTEXT.md]
